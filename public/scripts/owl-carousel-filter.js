...
$(document).ready(function () {
  ...
  $('.timeline-carousel .owl-item').click(function () {
    ...
    $('#' + activeTab).fadeIn();
    setTimeout(function () {
      $('#' + activeTab + '.timeline-carousel .owl-item').first().children().find(".year").trigger('click');
    }, 100);
    return false;
  });
  $('#awards-main li a').click(function () {
    var tabName = $(this).attr("href");
    //console.log(tabName)
    $(tabName + ' .timeline-carousel .owl-item').first().children().find(".year").trigger('click');
  })


  $('.gallery-owl-carousel').on('initialized.owl.carousel changed.owl.carousel', function (e) {
    if (!e.namespace) {
      return;
    }
    var carousel = e.relatedTarget;
    //console.log($(this).next());
    $(this).next().text(carousel.relative(carousel.current()) + 1 + '/' + carousel.items().length);
  }).owlCarousel({
    loop: false,
    dots: false,
    nav: true,
